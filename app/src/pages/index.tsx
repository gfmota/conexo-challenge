  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <div className={styles.topicForms} key={topicValue}>
        <input className={styles.textInput} {...register(`topic${topicValue}`)} placeholder={`Tópico ${topicValue}`} />
        <div className={styles.wordForms}>
          {[1,2,3,4].map((wordValue) => (
            <input className={styles.textInput} key={wordValue} {...register(`word${topicValue}${wordValue}`)} placeholder={`Palavra ${wordValue}`} />
          ))}
          </div>
      </div>
      ))}
